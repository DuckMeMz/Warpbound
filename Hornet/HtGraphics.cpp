...
std::vector<PictureIndex> HtGraphics::LoadSpritesheet(const std::string filename, int spriteRes, int frameheight, int framewidth)
{
    //Load image onto surface
    SDL_Surface* pImageSurface = IMG_Load(filename.c_str());
    if (pImageSurface == nullptr) {
        ErrorLogger::Write("Failed to load image: " + filename);
        return {}; //Empty vector for errors
    }

    SDL_SetColorKey(pImageSurface, SDL_TRUE, SDL_MapRGB(pImageSurface->format,
        m_transparentColour.r, m_transparentColour.g, m_transparentColour.b));

    std::vector<PictureIndex> frames;

    //First loop iterates over the rows of the spritesheet
    for (int i = 0; i < frameheight; i++)
    {
        //Second loop iterates over the columns
        for (int j = 0; j < framewidth; j++)
        {
            //This is the rect that defines the cropped area
            SDL_Rect sourceRect = { j * spriteRes, i * spriteRes, spriteRes, spriteRes };

            //Create a surface of the sprite's resolution
            SDL_Surface* pCroppedSurface = SDL_CreateRGBSurfaceWithFormat(0, spriteRes, spriteRes, 32, pImageSurface->format->format);
            if (pCroppedSurface == nullptr) {
                ErrorLogger::Write("Failed to create cropped surface");
                SDL_FreeSurface(pImageSurface);
                return {}; 
            }

            // Copy the selected part of the image into the cropped surface
            SDL_BlitSurface(pImageSurface, &sourceRect, pCroppedSurface, nullptr);

            // Create the texture from the cropped surface
            SDL_Texture* pCroppedTexture = SDL_CreateTextureFromSurface(m_pRenderer, pCroppedSurface);
     

            Texture texture;
            texture.pTexture = pCroppedTexture;
            if (texture.pTexture == nullptr)
            {
                ErrorLogger::Write("Could not create texture image in HtGraphics::LoadSpritesheet():" + filename);
                ErrorLogger::Write(SDL_GetError());
                return {};
            }
            texture.width = pImageSurface->w;
...
PictureIndex HtGraphics::LoadSpritesheetFrame(const std::string filename, int spriteRes, int rows, int columns, int pixelMargin, int iteration, int loadOrder, bool isStar)
{
    if (isStar) spriteRes *= 2;

    if (iteration >= (rows * columns)) iteration = 0;

    try
    {
        pCurrentSpritesheet = m_pSpritesheets.at(loadOrder);
    }
    catch (const std::out_of_range&)
    {
        pCurrentSpritesheet = IMG_Load(filename.c_str());
        m_pSpritesheets.push_back(pCurrentSpritesheet);
    }
    pCurrentSpritesheet = m_pSpritesheets[loadOrder];
   
    SDL_SetColorKey(pCurrentSpritesheet, SDL_TRUE, SDL_MapRGB(pCurrentSpritesheet->format, m_transparentColour.r, m_transparentColour.g, m_transparentColour.b));

    int column = iteration % columns;
    int row = iteration / columns;

    //This is the rect that defines the cropped area
    SDL_Rect sourceRect = { column * spriteRes, row * spriteRes, spriteRes, spriteRes };

    //Create a surface of the sprite's resolution
    SDL_Surface* pCroppedSurface = SDL_CreateRGBSurfaceWithFormat(0, spriteRes, spriteRes, 32, pCurrentSpritesheet->format->format);
 
    // Copy the selected part of the image into the cropped surface
    SDL_BlitSurface(pCurrentSpritesheet, &sourceRect, pCroppedSurface, nullptr);
    ...
    Texture texture;
    texture.pTexture = pCroppedTexture;
    texture.width = spriteRes;
    texture.height = spriteRes;

    //3600 is the buffer for static objects. More than enough

    PictureIndex oldIndex = static_cast<PictureIndex>(loadOrder * (rows * columns) + iteration + 3600);

    if (m_pictureMap.insert(std::pair<PictureIndex, Texture>(oldIndex, texture)).second == false)
    { 
        ErrorLogger::Write("Tried inserting on key that already existed " + std::to_string(static_cast<int>(m_nextPictureIndex)));
    }

    pCurrentSpritesheet = nullptr;
...
Result HtGraphics::DrawRect(Rectangle2D rect, Colour colour, int thickness)
{
    rect = HtCamera::instance.NativeTransform(rect);

    SDL_Rect nonFillRect;
    nonFillRect.x = int(rect.GetBottomLeft().XValue);
    nonFillRect.y = int(rect.GetBottomLeft().YValue);
    nonFillRect.w = int(rect.GetTopRight().XValue - rect.GetBottomLeft().XValue);
    nonFillRect.h = int(rect.GetTopRight().YValue - rect.GetBottomLeft().YValue);
    ...
    for (int i = 0; i < thickness; ++i)
    {
        ...
        r.x = nonFillRect.x - i;
        r.y = nonFillRect.y - i;
        r.w = nonFillRect.w + i * 2;
        r.h = nonFillRect.h + i * 2;

        SDL_SetRenderDrawColor(m_pRenderer, colour.r, colour.g, colour.b, colour.a);
        if (SDL_RenderDrawRect(m_pRenderer, &r) < 0)
        {
...
Result HtGraphics::DrawCircle(Circle2D circle, Colour colour)
{
    return DrawCircle(circle.GetCentre(), circle.GetRadius(), colour);
}

Result HtGraphics::DrawCircle(Vector2D centre, double radius, Colour colour)
{
    centre = HtCamera::instance.NativeTransform(centre);
    radius = HtCamera::instance.NativeTransform(radius);
    SDL_SetRenderDrawColor(m_pRenderer, colour.r, colour.g, colour.b, colour.a);
    ...
    while (offsety >= offsetx)
    {
        error += SDL_RenderDrawPoint(m_pRenderer, x + offsetx, y + offsety);
        error += SDL_RenderDrawPoint(m_pRenderer, x + offsety, y + offsetx);
        error += SDL_RenderDrawPoint(m_pRenderer, x - offsetx, y + offsety);
        error += SDL_RenderDrawPoint(m_pRenderer, x - offsety, y + offsetx);
        error += SDL_RenderDrawPoint(m_pRenderer, x - offsetx, y - offsety);
        error += SDL_RenderDrawPoint(m_pRenderer, x - offsety, y - offsetx);
...
{
    //Releases all spritesheet surfaces
    ReleaseAllSpritesheets();
...
{
    if (m_pictureMap.find(picIndex) == m_pictureMap.end()) {
        ErrorLogger::Write("PictureIndex not found in HtGraphics::DestroyTexture(PictureIndex)!");
    }
    else 
    {
        Texture tempTex = m_pictureMap[picIndex];
        if (tempTex.pTexture == nullptr)
        {
            ErrorLogger::Write("Texture nullptr in HtGraphics::DestroyTexture(PictureIndex)!"); 
        }
        else
        {
            SDL_DestroyTexture(tempTex.pTexture);
        }
    } 
}

void HtGraphics::DestroyTexture(Texture texture)
{
    if (texture.pTexture == nullptr)
    {
        ErrorLogger::Write("Texture nullptr in HtGraphics::DestroyTexture(Texture)!");
    }
    else
    {
         SDL_DestroyTexture(texture.pTexture);
...
void HtGraphics::DeletePictureMapElement(PictureIndex picIndex)
{
    std::map<PictureIndex, Texture>::iterator picit = m_pictureMap.find(picIndex);
    if (picit == m_pictureMap.end())
    {
        ErrorLogger::Write("Invalid PictureIndex in HtGraphics::DeletePictureMapElement():" + std::to_string(int(picIndex)));
    }
    else
    {
        m_pictureMap.erase(picit);
...
{
    //Checks if font already loaded
    auto it = m_fontFilenameMap.find(filename);

    if (it != m_fontFilenameMap.end())
    {
        //If it is then return the FontIndex
        return it->second;
    }
