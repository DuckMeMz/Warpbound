...
void HtMouse::FakeLeftClick()
{
    if (m_pMouseListener)
    {
        m_pMouseListener->HandleMouseButtonEvent(HtMouseButton::LEFT, true);
        m_pMouseListener->HandleMouseButtonEvent(HtMouseButton::LEFT, false);
    }
    m_leftDown = false;
    m_oldLeftDown = false;
}
...
Vector2D HtMouse::GetPointerPosition() const
{
    return Vector2D(m_pointerX, m_pointerY);
}
