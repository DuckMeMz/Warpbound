...
public:
    ...
    std::vector<PictureIndex> LoadSpritesheet(const std::string filename, int spriteRes, int frameheight, int framewidth);

    PictureIndex LoadSpritesheetFrame(const std::string filename, int spriteRes, int rows, int columns, int pixelMargin, int iteration, int loadOrder, bool isStar);

    void ClearSpritesheets();

    //std::vector<PictureIndex> LoadSpritesheet(const std::string& filename, int framesWidth, int framesHeight, int pixels);
    ...
    Result DrawRect(Rectangle2D rect, Colour colour, int thickess);
    ...
    Result DrawCircle(Circle2D circle, Colour colour);
    Result DrawCircle(Vector2D centre, double radius, Colour colour);
    ...
    void ReleaseAllSpritesheets();

    void DestroyTexture(PictureIndex picIndex);

    void DestroyTexture(Texture texture);

    void DeletePictureMapElement(PictureIndex picIndex);
...
private:
    ...
    //Spritesheet related variables
    SDL_Surface* pCurrentSpritesheet = nullptr;
    std::vector<SDL_Surface*> m_pSpritesheets;              // Vector of spritesheet surfaces
    ...
    std::map<std::string, FontIndex> m_fontFilenameMap;     // Map of font filenames
